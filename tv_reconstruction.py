...
for filename in os.listdir(datadirectory):
    if filename.endswith(".png"):
        ...
        xtv, errtv = regularizer.TV(co.project(FBP(data)), data, niter=niterTV, alpha=alphaTV)
        xtv = xtv.data
        co.error_plot(errtv, 'TVError', savepath)

        nmse = co.NMSE(ph, co.project(xtv))

        psnr_list.append(co.PSNR(ph, co.project(xtv)))
        nmse_list.append(nmse)

        plot(xtv, '{:.2e}'.format(nmse), savepath=savepath, name='TV')
